import { Component, OnInit } from '@angular/core';
import { ToastrService } from 'ngx-toastr';
import {ERPService} from '..//erp.service';          
import { FormBuilder,FormGroup } from '@angular/forms'; 
import {ReserveModComponent} from 'src/app/reserve-mod/reserve-mod.component'

@Component({
  selector: 'app-reserve',
  templateUrl: './reserve.component.html',
  styleUrls: ['./reserve.component.sass']
})
export class ReserveComponent implements OnInit {
  Reserves: object;
  AddForm: FormGroup;
  NewReserve:object;
  nReserve:object;
  constructor(private toastrService: ToastrService, private data: ERPService, private formBuilder: FormBuilder,private mod:ReserveModComponent) { }

  ngOnInit() {
    this.AddForm = this.formBuilder.group({
      Name: [],
      Description: [],
      Lattitude: [],
      Longitude: []
      });
    this.data.GetReserves().subscribe(res=>{
      this.Reserves =res;
    })
  }
  add(){
    var Name = this.AddForm.get('Name').value;
    var Description = this.AddForm.get('Description').value;
    var Lattitude = this.AddForm.get('Lattitude').value;
    var Longitude = this.AddForm.get('Longitude').value;
    if ( Name ==""||Description==""||Lattitude==""||Longitude=="") {
      document.getElementById("inputErr").click();
    }
    else {
      this.nReserve = {
        "Name": Name,
        "Description": Description,
        "Lat": Lattitude,
        "Lng": Longitude
      };
      console.log(this.nReserve);
      this.data.PostReserve(this.nReserve).subscribe(res => {
        if (res != null)
        {
          this.ngOnInit();
          this.showToast();
        }
        else
        {
          document.getElementById("inputErr").click();
        }
        
      });
    }
  }
  edit(ID){
    this.mod.edit(ID);
  }
  showToast(){
    this.toastrService.show("Record added successfully.", "Success!");
  }
  delToast(){
    this.toastrService.show("Record could not be removed.", "Error!");
  }
  delSuccessToast(){
    this.toastrService.show("Record removed.", "Success!");
  }
  delete(ID){
    this.data.nID = ID;
    document.getElementById('del').click();
  }
  del(){
    this.data.DeleteReserve(this.data.nID).subscribe(res=>{
      if (res!=null)
      {
        this.delSuccessToast();
        this.ngOnInit();
      }
      else if (res==2)
      {
        alert("You are not allowed to delete this record");
      }
      else
      {
        this.delToast()
      }
    })
  }
}
